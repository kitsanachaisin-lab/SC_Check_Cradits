...
def course_classify(course_dict:dict, distinct:bool, degree:str ):
    ...
    # Course filter
    gened_uni, enrolled_course = filter_course(enrolled_course=enrolled_course, reference=gened_uni,degree=degree)
    gened_dep, enrolled_course = filter_course(enrolled_course=enrolled_course, reference=gened_dep,degree=degree)
    course_core, enrolled_course = filter_course(enrolled_course=enrolled_course, reference=course_core,degree=degree)
    course_math, enrolled_course = filter_course(enrolled_course=enrolled_course, reference=course_math,degree=degree)
    gened_opt, course_opt, elec_course = filter_course(enrolled_course=enrolled_course,degree=degree)
...
def filter_course(enrolled_course:list, reference:list = None, degree:str = None):
    degree_prefix_map = {
        "Math": "SCMA",
        "Plant": "SCPL",
        "Chem": "SCCH",
    ...
    if reference is not None:
        ...
        course_opt = list(filter(lambda enroll: enroll[0][:4] == degree_prefix_map.get(degree, ""), enrolled_course))
...
def get_information(courses_data:dict, is_distinct:bool, degree:str = None):
    classified_dict = course_classify(course_dict=courses_data, distinct=is_distinct,degree=degree)
